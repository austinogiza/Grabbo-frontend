...
const BlogDetailed = () => {
  ...
  const fetchComment = () => {
    ...
    axios
      .get(commentFetchURL, { params: { slug } })
      .then((res) => {
        ...
        <>
          ...
          {data && (
            <>
              ...
              <BlogDetails
                className="container max-w-6xl mx-auto mt-4 mb-4 min-h-0 h-full"
          ...
          <Container className="mx-auto container max-w-6xl flex flex-col">
            <CommentSection className="container max-w-6xl mx-auto my-4 mb-4 h-auto flex flex-col min-h-0">
              <CommentHeader className="h-12 my-4">
                <Commenttitle className="text-black text-4xl font-semibold">
                  {data.comments ? data.comments.length : 0}{" "}
                  {data.comments && data.comments.length > 1
                    ? "Comments"
                    : "Comment"}{" "}
                  ...
                  <>
                    {commentData.map((comment) => (
                      <Commentbox className="my-4 w-full h-full min-h-0 py-4 px-4 bg-gray-100 rounded-md">
                        ...
                        <Commenttext className="text-base font-normal text-gray-500 mt-3 text-left">
                          {comment.comment}
                        </Commenttext>
                      </Commentbox>
                    ))}
                  ...
                  <>
                    <Nocomment className="my-4 flex justify-center items-center w-full">
                      <p className="text-md font-medium">
                        No comment on this post yet. Kindly drop a comment
                      </p>
                    </Nocomment>
                  </>
                )}
              ...
              <NotCommented>
                <form
                  className="space-y-6 max-w-lg mx-auto my-4"
                  ...
                  <div class="flex flex-col ">
                    ...
                    <div class="mt-2">
                      <textarea
                        ...
                        value={comment}
                        rows="6"
                        class="max-w-lg resize-none shadow-sm block w-full focus:ring-indigo-500 focus:border-indigo-500 py-2 px-2 focus:outline-none sm:text-sm border border-gray-300 rounded-md"
                      ></textarea>
